  ...
  return (
    ...
    >
      ...
      <ModalComponent
        ...
        handleClose={() => handleClose('adulto')} image={adultotreino} description={'O Taekwondo é uma excelente forma de se manter ativo e saudável, além de melhorar o foco e a fortaleza mental. Temos alunos de todas as idades que nos contam que notaram grandes melhorias na sua concentração e autoconfiança.'}      />
      <ModalComponent
        title="Crianças"
        open={openModal.criancas}
        handleClose={() => handleClose('criancas')} image={treinocriana}  description={'O Taekwondo vai além de uma simples arte marcial; é uma jornada de crescimento físico e mental para crianças. Primeiramente, promove uma melhoria significativa na saúde física e na coordenação motora, essencial durante os anos de crescimento. Além disso, ao praticar em grupo, as crianças desenvolvem habilidades sociais e aprendem o valor do trabalho em equipe, preparando-as para desafios futuros.'}      />
      <ModalComponent
        title="Funcional"
        open={openModal.funcional}
        handleClose={() => handleClose('funcional')} image={funcionalImage} description={'O treinamento funcional melhora a força muscular e a resistência, preparando o corpo para as demandas diárias e reduzindo o risco de lesões. A natureza dinâmica desses exercícios também melhora significativamente a coordenação, agilidade e equilíbrio, fundamentais para uma vida ativa e saudável.'}       />
    </Box>
  );
}

const CardComponent: React.FC<CardComponentProps> = ({ title, image, onClick }) => {
  const theme = useTheme();
  return (
...
const ModalComponent: React.FC<ModalComponentProps> = ({ title, open, handleClose,image,description }) => {
  return (
    <Modal
    open={open}
    onClose={handleClose}
